...
from .models import CHANNEL_LABELS, SismogramRecord, any_record_has_vibration_alert, get_primary_client
...
def _vibration_status_text(records: list[SismogramRecord], threshold_mm_s: float) -> str | None:
    ...
    if any_record_has_vibration_alert(records, threshold_mm_s):
        return f"⚠️ Índices de vibração: acima de {format_decimal(threshold_mm_s, 1)} mm/s."
